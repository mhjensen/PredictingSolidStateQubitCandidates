...
\mrk{Note that this data mining process alone was intended for further experimental studies and not necessarily for a targeted machine learning search. Nonetheless, we adapt the selection criteria from Ref.~\cite{Ferrenti2020} for the present purposes.}
In the \mrk{Ferrenti approach} framework, \textbf{suitable} candidates \mrk{are labeled} by the following steps: 
\begin{enumerate}
...
\mrk{Performing machine learning on a data set derived using preconceived notions for which material properties are important is likely to reproduce several of the initial selection criteria. Nonetheless, the Ferrenti approach was included in the present work to highlight expectations from the literature and contrast them with the findings from the other approaches. Additionally, one should note that ML methods are capable of recognizing other patterns than those intended for the data, opening up the possibility that also the Ferrenti approach could yield new insights.} 
...
For \textbf{unsuitable} candidates, \mrk{the same strategy as for the Ferrenti approach was implemented}. The result is a somewhat unbalanced set of labeled materials with up to $75 \ \%$ of the materials found in the suitable group\mrk{ and $78 \ \%$ more materials than for the Ferrenti approach.} 
...
\begin{figure}[ht]
    ...
    \centering
    \input{figures/histograms/revised/LegendHistogram.tex}
  \end{subfigure}
  \par\bigskip
\begin{subfigure}[b]{0.45\textwidth}
    \input{figures/histograms/revised/MP_Eg_count_075.tex}
    \subcaption{}
\end{subfigure}
\begin{subfigure}[b]{0.45\textwidth}
    \input{figures/histograms/revised/IonProperty_max ionic char_count_075.tex}
    \subcaption{}
\end{subfigure}%

\begin{subfigure}[b]{0.45\textwidth}
    \input{figures/histograms/revised/GeneralizedRDF_std_dev Gaussian center=1.0 width=1.0_count_075.tex}
    \subcaption{}
\end{subfigure}
\begin{subfigure}[b]{0.45\textwidth}
    \input{figures/histograms/revised/AverageBondLength_std_dev Average bond length_count_075.tex}
    \subcaption{}
\end{subfigure}
