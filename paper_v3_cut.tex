...
Semiconductor materials provide a compelling platform for quantum technologies (QT), and the properties of a vast amount of materials can be found in databases containing information from both experimental and theoretical explorations. However, searching these databases to find promising candidate materials for quantum technology applications 
%among tens of thousands of contenders
...
Conversely, defining materials as unsuitable candidates for QT is in many ways equally difficult.
This is similar to the case for the suitable candidates in that it is not understood which mechanisms break the quantum properties.
However, we can make some educated guesses and assume that the negation of the criteria we use to select suitable candidates should give unsuitable candidates.
A side-effect of this selection is that the criteria for unsuitable candidates can become equally, or more, restrictive compared to the criteria for suitable candidates (we would assume that most materials are not optimal for the kind of QT we are concerned with in this article).
We will then find ourselves in a situation with skewed data sets were we have fewer unsuitable than suitable candidates.
This will not be a perfect method, but it will serve as a starting point and demonstration for how the labeling procedure could be improved.
...
Here we see the first example where the criteria for the unsuitable candidates is more restrictive for our data set than the criteria for the suitable candidates.
...
%, in contrast to the case for the two Ferrenti  approaches, 
...
%The Ferrenti approach also admits a large amount of materials from the training set as compared to the empirical approach, but the resulting data are otherwise not very different from the extended version, whereas the empirical approach predicts fewer suitable candidates that are possible to verify manually. 
...
%However, the amount is yet too  The resulting predictions are not very different from the extended version, whereas the empirical approach predicts fewer suitable candidates that are possible to verify manually. 
...
%Our findings are corroborated by the fact that all four ML methods exhibit similar results for all datasets. 
