...
In Table~\ref{tab:01-pc}, we include the precise measurements for each of the evaluation metrics for the optimal number of principal components. The optimal number is visualized as dotted lines in Fig.~\ref{fig:01-pca}. The relevant hyperparameters for logistic regression were the maximum iterations. The number of iterations was set at $400$, and the regularization term was found optimal at $0.46$. For random forests and decision trees, we find the maximum depth to be $7$. Gradient boosting, which normally uses a weak learner, had a decision tree depth of $4$. We find the best performing method to be logistic regression, but this method depends on a large number of principal components. 
...
Similar to the Fererenti approach, the extended Ferrenti finds the DFT band gap of the elemental solids among the elements in the composition important. The feature regarding the band gap originates from the highest value from the first principal component. 
...
The grid search for the optimal number of principal components is visualized in Figure~\ref{fig:03-pca} for the empirical approach. Interestingly, we find that all ML methods  experience high scores for just a few principal components, where 
%$1$
the first
